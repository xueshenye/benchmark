...
实证倾向:愿意主动澄清的模型更容易全轮通过;不问就猜的模型常在 M3(格式细节)与 M4(反转作用域 / check 精度)栽跟头。完整结果见 §5(待最终多模型 grid 完成后更新)。
...
- **单元测试**(无 Docker):`tests/` **97 项全通过** —— 控制器 / 澄清子循环 / 人工模式 / 每任务 scorer 一致性 + reward 双模式。

## 5. 模型运行结果

> 待最终多模型 grid 完成后更新。当前进度见 [PROJECT_STATE.md](PROJECT_STATE.md) §5。

| 模型 / 后端 | reward | 逐轮 (R1–R4) | 澄清 / 纠正 / 强制 | judge-vs-scorer 分歧 | 时长 / 费用 |
|---|---|---|---|---|---|
| (待填) | | | | | |
...
├── .env.example                     # 运行凭证模板
└── CLAUDE.md / PROJECT_STATE.md     # 章程 / 交接文档
```

> **Design A+ 与 Design B**:devteam(以及 T1–T3)走 **Design A+** —— 自定义 agent 在单 trial 内驱动动态轮次(`InteractiveUserClaude` + `TurnController`)。**Design B**(`benchmark/{step_driver,multi_step_trial,interactive_step_agent,design_b_plugin}.py`)是实验性的**原生 multi-step** 路径:每步 = 一轮,步间 runner 读上一步轨迹 → user-LLM 生成下一步指令,用于 `tasks/benchmark/multi-round-cli-demo-multistep/`(仅该演示任务),纯增量、可回退。
...
$PY -m pytest tests/
```
期望:97 项全通过(控制器 / 澄清子循环 / 人工模式 / 各任务 scorer 一致性 + reward 双模式)。
...
2. **未进行多轮 MT@k 试验**:由于时间与算力限制,尚未按 EvoCode-Bench 的做法对同一任务做多轮重复采样(MT@k),无法评估单次随机性对排名的扰动。后续应在算力允许时补上。
3. **对用户的模拟还需提升可靠性与真实性**:实测 user-LLM 存在**过度接受**(judge-vs-scorer 分歧:user 判定满足但 verifier 判 0),且偶尔会"发明"ground-truth 之外的要求。需更强的忠实度护栏、judge-vs-scorer 分歧报告作为质量哨兵,并用多 user-LLM 模型校准("Lost in Simulation" 敏感性)。真人扮演模式(`USER_SIMULATOR=manual`)可作为高可靠性的替代与校验手段。
